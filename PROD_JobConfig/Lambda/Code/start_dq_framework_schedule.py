...
def ddb_update_records(**kwargs):
    try:
        ...
        if kwargs['partKey'] == "DQ-Framework":
...
def sns_notification(sns_arn, msg, task_status, task_date):
    try:
        response = sns_client.publish(
            TopicArn=sns_arn,
            Message=msg,
            Subject='ODP DQ Framework Status ' + task_status + ' - ' + task_date
        )
        return True
    ...
    try:
        ...
        if validate_date_repsonse:
            try:
                ...
                ddb_second_response = ddb_get_unique_record(dqf_partKey, dqf_sortKey)
                ...
                if ddb_second_response:
                    ...
                    if len(check_n_count) == 1:
                        ...
                        print("All completed and running the DQF")
                        msg = "IB Intel CDS, SR CDS & CI CDS all are complted starting DQ Framework..."
                        ...
                        msg = "{} only completed yet to start DQ Framework".format(TRACK_NAME)
