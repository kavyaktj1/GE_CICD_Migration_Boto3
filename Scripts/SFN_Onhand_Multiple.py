...
filename = 'Scripts/SFN_Details_MAPP_Onhand.csv'
    ...
    for sfn_ind in range(size):
        ...
        try:
            if sfn_ind!=0:
                ...
                if get_time_diff(end_time_before,start_time_current)>30:
...
for DF in DF_list:
    ...
    for index in range(len(html_string_lines)):
        ...
        elif '<td>ONHAND</td>' in line:
            html_string_lines[index] = line.replace('<td>','<td style="border: 1px solid black;padding: 10px;background-color:#873600;white-space:nowrap">')
        elif '<th>' in line:
