...
def update_target_paths(targets):
    if 'JdbcTargets' in targets:
        jdbc = targets['JdbcTargets']
        for index in range(len(jdbc)):
            if 'ConnectionName' in jdbc[index]:
                jdbc[index]['ConnectionName'] = 'us-nprod-peering-redshift'
        targets['JdbcTargets'] = jdbc
...
for index in range(size):
    with open(path_prefix+'/Backup/PROD/Crawler/'+ df['Crawler'][index] + '.json', 'r') as f: # Getting the crawler config from Git.
        crw = json.load(f)
