...
for index in range(size):
    with open(path_prefix+'/Backup/PROD/Lambda/'+df['Lambda'][index]+'.json','r') as json_file: # Gets the config file for the job.
        func_config = json.load(json_file)
    with open(path_prefix+'/Backup/PROD/Lambda/Code/'+df['Lambda'][index]+'.py', 'r') as f: # Storing the initial config in the 'Input' folder.
        python_code = f.read()
    ...
    shutil.copy(path_prefix+'/Backup/PROD/Lambda/Code/'+df['Lambda'][index]+".py",config['Python_file_name']) # Moves and renames the python code file as 'lambda_function.py'
