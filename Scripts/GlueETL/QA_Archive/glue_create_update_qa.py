# The script will retrieve the updated PROD Config from Git and create or update the Glue Job in QA Environment.
...
def modify_job(glue_job,pr_id,track): # This function takes job config as argument and modifies the respective job.
    ...
    with open(config['Rollback_Path']+job['Name']+'-'+pr_id+'.json', 'w',encoding = 'UTF-8') as outfile: # Before modification, current config is stored in Git as 'Rollback'.
...
for index in range(size):
    ...
    job = job.strip()
    with open(config['JSON_File_QA']+job+'.json') as f: # Gets the updated file config for PROD Environment from Git.
        JOB = json.load(f)
        try:
