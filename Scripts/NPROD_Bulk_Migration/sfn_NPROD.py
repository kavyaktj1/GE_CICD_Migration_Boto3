...
df = pandas.read_csv('../StepFunction/'+config['job_name_list'])
...
def create_state_machine(jobname,pr_id): # This function will create the SFN with function name as argument.
    try:
        with open(path_prefix+'/Backup/PROD/StepFunction/'+jobname+'.json','r') as json_file: # Gets the SFN Definition from the Git.
            main = json.load(json_file)
            def_data_str = main['definition']
            def_data = json.loads(def_data_str)
        ...
        print('Getting exception while SFN creation for : ',jobname,e)
    
def modify_state_machine(job,name,pr_id): # This function will modify the SFN with function name and config as arguments.
    try:
        with open(path_prefix+'/Backup/PROD/StepFunction/'+name+'.json','r') as json_file: # Gets the SFN Definition from the Git.
            main = json.load(json_file)
            def_data_str = main['definition']
            def_data = json.loads(def_data_str)
        ...
        print('Getting exception while updating function : ',name,e)
