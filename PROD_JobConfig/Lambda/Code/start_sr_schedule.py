...
def ddb_update_records(**kwargs):
    try:
        ...
        if kwargs['partKey'] == "SR-Entity" or kwargs['partKey'] == "DQ-Framework":
    ...
    try:
        ...
        if validate_date_repsonse:
            try:
                ...
                sr_partKey, sr_sortKey = 'SR-Entity', validate_date_repsonse
                dqf_partKey, dqf_sortKey = 'DQ-Framework', validate_date_repsonse

                print("sp {}, sk {}".format(sr_partKey, sr_sortKey))
                print("dqpk {}, dqsk {}".format(dqf_partKey, dqf_sortKey))

                ddb_second_response = ddb_get_unique_record(sr_partKey, sr_sortKey)
                ...
                if ddb_second_response:
                    ...
                    ddb_default_data = {
                        ...
                        "CI_SR_ENTITY": ddb_second_response['CI_SR_ENTITY']
                    }
                    ...
                    if len(check_n_count) == 1:
                        ...
                        if TRACK_NAME == "IB_INTEL_CDS":
                            ddb_dqf_response = ddb_get_unique_record(dqf_partKey, dqf_sortKey)

                            ddb_dqf_data = {
                                "partKey": ddb_dqf_response["partKey"],
                                "sortKey": ddb_dqf_response["sortKey"],
                                "IB_INTEL_CDS": ddb_dqf_response["IB_INTEL_CDS"],
                                "SR_CDS": ddb_dqf_response["SR_CDS"],
                                "CI_CDS": ddb_dqf_response["CI_CDS"]
                            }

                            ddb_dqf_data.update({TRACK_NAME: "Y"})
                            ddb_update_records(**ddb_dqf_data)
                            print("DQF Response {}".format(ddb_dqf_data))

                        msg = "CI SR Entity & IB INTEL CDS loads are completed and starting SR Entity Load"
                        ...
                        msg = "{} only completed yet to start SR Entity Load".format(TRACK_NAME)
