...
df = pandas.read_csv('../GlueETL/'+config['classifier_name_list'])
...
for index in range(size):
    name = df['Name'][index]
    with open(path_prefix+'/Backup/PROD/GlueETL/Classifier/'+name+'.json') as f: # Gets the updated file config for PROD Environment from Git.
        JOB = json.load(f)
        try:
