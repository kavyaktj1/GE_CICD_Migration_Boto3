...
def change_source_db(defarg): # From 'source_db', modifies 'mirror' to 'mrr'.
    for key in defarg.keys():
        if(key.find('source_db')!=-1):
            if(defarg[key].find('mirror')!=-1):
                value = defarg[key].split('mirror')
                defarg[key] = value[0]+'mrr'+value[1]
    return defarg
...
def change_script_loc(command,track,filename): # Changes the Track and prefix in script location path.
    ...
    if track=='Entity':
        newloc = 's3://odp-us-prod-raw/servicesuite/ODP/Common/src/glue/Entity/'+f_name
    elif track=='DQ_Framework':
        newloc = 's3://odp-us-innovation-servicesuite/common/502818085-CICD/ODP_DEV/ODP/Common/src/glue/DQ_Framework/'+filename+'.py'
    elif track=='Compaction':
        newloc = config['Script_location_prefix']+f_name
    else:
        newloc = 's3://odp-us-prod-raw/servicesuite/ODP/'+track+'/src/glue/'+f_name
...
def modify_shell_params(defarg):
    ...
    if '--redshift_secret_key' in defarg:
        if 'daasbatchextract' in defarg['--redshift_secret_key']:
            defarg['--redshift_secret_key'] = 'us-prod-daasbatchextract-fsso'
        else:
...
for index in range(size):
    ...
    daas_flag = False
    with open(path_prefix+config['JSON_File_DEV']+track+'/'+job+'.json','r') as f: # Getting the config file from Git.
        ...
        time.sleep(0.5)
        if config['daas_job_role_check'] in glue_job['Role']:
            glue_job['Role'] = config['daas_iam_role']
            daas_flag=True
            print('DaaS job')
        else:
