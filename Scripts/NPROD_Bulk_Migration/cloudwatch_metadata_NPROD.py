...
df = pandas.read_csv('../CloudWatch/'+config['job_name_list'])
...
for index in range(size):  
    with open(path_prefix+'/Backup/PROD/CloudWatch/Events/'+df['CW'][index]+'.json','r') as f: # Reading the event config file from Git.
    ...
    if 'EventPattern' in resp_event:
        resp_event['EventPattern'] = resp_event['EventPattern'].replace('245792935030',config['PROD_Acc'])
    if 'daasbatchextract' not in resp_event['Name']:
        resp_event['State'] = 'DISABLED'  
    ...
    with open(path_prefix+'/Backup/PROD/CloudWatch/Targets/'+ df['CW'][index] + '.json', 'r') as f: # Reading the target config file from Git.
        targets = json.load(f)
    ...
    for i in range(len(targets)): # Updating the target config
        ...
        name = targets[i]['Arn']
        targets[i]['Arn'] = name.replace('245792935030',config['PROD_Acc'])
