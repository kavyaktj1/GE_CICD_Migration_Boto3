...
df = pandas.read_csv('job_list_nprod.csv')
...
if len(glueclassifier_prod)>=0:
    service = 'GlueETL'
    csv_file_name = 'classifier_dev_nprod.csv'
    csv_file_path = service+'/'+csv_file_name
    header_name = 'Name'
    header=[header_name,'PR_ID']
...
if len(dynamodb_prod)>=0:
    service = 'DynamoDB'
    csv_file_name = 'dynamo_dev_nprod.csv'
    csv_file_path = service+'/'+csv_file_name
...
if len(sp_list)>=0:
    csv_file_name = 'redshift_sp_nprod.csv'
    csv_file_path = csv_file_name
    header = ['Name','Track','Type']
    write_to_csv(header,sp_list,csv_file_path)
    
if len(ddl_list)>=0:
    csv_file_name = 'redshift_ddl_nprod.csv'
    csv_file_path = csv_file_name
