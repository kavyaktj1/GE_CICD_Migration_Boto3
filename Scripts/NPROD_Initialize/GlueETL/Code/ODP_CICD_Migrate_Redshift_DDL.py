...
def get_manifest_file():
    ...
    location_obj = s3.Object("odp-us-nprod-servicesuite",
                             "common/502818085/ODP/manifest/manifest_ddl_git_info.json")
...
def get_pre_manifest_file():
    s3 = boto3.resource('s3', region_name='us-east-1')
    location_obj = s3.Object("odp-us-nprod-servicesuite",
                             "common/502818085/ODP/manifest/manifest_ddl_order.json")
    manifest_file = json.loads((location_obj.get()['Body'].read().decode('utf-8')))
    return manifest_file
...
def comp_git_changelog(manifest):
    try:
        del_items = {}
        exec_items = {}
        con = connect( host='us-nprod-redshift.cjvhsrduwbym.us-east-1.redshift.amazonaws.com', user=REDSHIFT_USERNAME, password=REDSHIFT_PASSWORD, database='gehc_data',port= '5439' )
        cur = con.cursor()
...
if __name__ == '__main__':
    try:
        ...
        for table_name_with_version,comp_path_with_pr in manifest_pre.items():
            ...
            s3_path = 'common/502818085/'+comp_path
            file_flag = 'NORMAL'
