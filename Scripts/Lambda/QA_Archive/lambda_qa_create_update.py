...
def update_lambda(config_file,zip_code,py_file,pr_id): # This function will update the lambda function config as well as code in PROD Environment.
    ...
    with open(config['Rollback_prefix']+config_file['FunctionName']+'-'+str(pr_id)+'.json', 'w',encoding = 'UTF-8') as outfile:
...
for index in range(size):
    ...
    file = open(config['Zip_Code_prefix']+df['Lambda'][index]+'.zip','rb') # Gets the Zipped code from Git.
    zip_code = file.read()
    ...
    with open(config['Config_prefix']+job+'.json','r') as f: # Gets the updated function config from Git.
        config_file = json.load(f)
    config_file['Role'] = config['RoleArn']
    with open(config['Config_output_prefix']+df['Lambda'][index]+'.json', 'w',encoding = 'UTF-8') as outfile: # Stores the updated function config in 'Output' folder.
