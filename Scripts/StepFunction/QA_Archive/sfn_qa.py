...
# and create/update the SFN in QA.
...
with open('../config.json', 'r') as f:
    data = json.load(f)
    config = data['StepFunctionConfigQA']
...
def create_state_machine(prefix,jobname,pr_id): # This function will create the SFN with function name as argument.
    with open(config['JSON_File_Prefix'] + jobname + '.json', 'r') as json_file: # Gets the SFN Definition from the Git.
        def_data = json.load(json_file)
    with open(config['JSON_File_DEV_Git'] + jobname + '.json', 'w', encoding='UTF-8') as outfile: # Stores the initial config in 'Input' Folder.
            ...
            'destinations': [
                {
                    'cloudWatchLogsLogGroup': {
                        'logGroupArn': config['cw_log_group_arn_prefix'] + config['cw_log_group_prefix'] + jobname +
                                       config['cw_log_group_postfix'] + ':*'
                    }
                },
            ]
        }
    )
    with open(config['JSON_File_Git'] +prefix+jobname+'-'+pr_id + '.json', 'w', encoding='UTF-8') as outfile: # Stores the final config in 'Output' Folder.
...
def modify_state_machine(job, name,pr_id): # This function will modify the SFN with function name and config as arguments.
    with open(config['JSON_File_Prefix'] + name + '.json', 'r') as json_file: # Gets the SFN Definition from the Git.
        def_data = json.load(json_file)
    with open(config['JSON_File_DEV_Git'] + name + '.json', 'w', encoding='UTF-8') as outfile: # Stores the initial config in 'Input' Folder.
        json.dump(def_data, outfile)
    load_params(def_data)
    updated_definition = json.dumps(def_data, indent=4) # Gets the updated definition.
    create_log_group(config['environment_prefix']+name)
    response = sfnClient.update_state_machine(
        ...
        loggingConfiguration={
            ...
            'destinations': [
                {
                    'cloudWatchLogsLogGroup': {
                        'logGroupArn': config['cw_log_group_arn_prefix'] + config['cw_log_group_prefix'] +config['environment_prefix']+name +
                                       config['cw_log_group_postfix'] + ':*'
                    }
                },
            ]
    ...
    with open(config['JSON_File_Git'] +config['environment_prefix']+ name +'-'+pr_id+ '.json', 'w', encoding='UTF-8') as outfile: # Stores the final config in 'Output' Folder.
        json.dump(updated_definition, outfile)
...
for index in range(size):
    ...
    try:
        response = sfnClient.describe_state_machine( # Checks if the SFN is already there or not.
            stateMachineArn=config['sfn_job_arn_prefix']+config['environment_prefix']+job
        )
        ...
        with open(config['Rollback_Path']+job+'-'+str(df['PR_ID'][index])+'.json', 'w', encoding='UTF-8') as outfile: # Stores the current config as 'Rollback' in Git.
            json.dump(response, outfile)
        modify_state_machine(config['sfn_job_arn_prefix']+config['environment_prefix']+job, job,str(df['PR_ID'][index]))
    except sfnClient.exceptions.StateMachineDoesNotExist as e: # If SFN is not found in Console, create_state_machine is called.
        create_state_machine(config['environment_prefix'],job,str(df['PR_ID'][index]))
