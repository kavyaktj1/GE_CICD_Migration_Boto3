...
def ddb_update_records(**kwargs):
    try:
        ...
        if kwargs['partKey'] == "NPS-Spotfire":
...
def sns_notification(sns_arn, msg, task_status, task_date):
    try:
        response = sns_client.publish(
            TopicArn=sns_arn,
            Message=msg,
            Subject='ODP NPS Spotfire Status ' + task_status + ' - ' + task_date
        )
        return True
    ...
    try:
        ...
        if validate_date_repsonse:
            try:
                ...
                print("sp {}, sk {}".format(nps_partKey, nps_sortKey))
                ddb_second_response = ddb_get_unique_record(nps_partKey, nps_sortKey)
                ...
                if ddb_second_response:
                    ...
                    ddb_default_data = {
                        ...
                        "CD_ENTITY": ddb_second_response['CD_ENTITY'],
                        "NPS_CDS": ddb_second_response['NPS_CDS']
                    }
                    ...
                    if len(check_n_count) == 1:
                        ...
                        msg = "CD Entity & NPS CDS loads are completed and starting NPS Spotfire Load"
                        ...
                        msg = "{} only completed yet to start NPS Spotfire Load".format(TRACK_NAME)
